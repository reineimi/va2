...
#va2tableWrap tr { display: flex; position: relative; margin: 0px; overflow: visible; transition: transform 0.3s; }
#va2tableWrap th,
#va2tableWrap td { background: var(--windowFg); border: 1px solid var(--itemFg); white-space: pre; text-align: left; position: relative; padding: 0.6em 0.75em; color: inherit; width: 16em; min-height: 3em; transition: transform 0.3s; }
#va2tableWrap th { font-weight: 700; }
#va2tableWrap textarea { all: unset; box-sizing: border-box; border: 0px solid var(--itemFg); transition: 0.3s; background: var(--windowFg); color: var(--fontColorX); position: absolute; top: 0px; left: 0px; padding: 0.6em 0.75em; width: 100%; height: 100%; }
#va2tableWrap textarea:hover,
#va2tableWrap textarea:focus { z-index: 1; transform: scale(1.1); border: 2px solid var(--green); }

#va2tableWrap .va2rowSel { transform: scale(0.9) translateX(2rem); }

#va2tableCleanup { min-height: 100%; min-width: 2.5em; margin: 0px 0.5rem 0px 0px; padding-top: 1px; }
...
#va2tableEditor { display: table; position: absolute; top: 0px; left: 0px; white-space: nowrap; padding: 1em 2em; margin: 0px 0px 3em 0px; background: var(--windowFg); border: 1px solid #aaa3; border-radius: 2em; }
#va2tableEditor * { display: inline-block; margin-right: 0.75em; }
#va2tableEditor button { background: none; font: inherit; font-weight: 600; cursor: pointer; padding: 0.5em 0.75em; border: 0.2rem solid #0000; border-radius: 1em; }
...
const va2tableEdit = create('button', {
	...
	onclick: ()=>{
		toggle('va2table', (state)=>{
			if (state) { // Add textarea mask to th/td
			...
			loop(tags('va2table', 'tr'), (i,tr)=>{
				if (i > 0) {
					...
					const btn = create(0, {
						className: 'red',
						innerHTML: `${va2.mod.tables.rows}`,
						onclick: ()=>{ rm(tr, btn); va2.mod.tables.rows--; }
					}, 'va2tableCleanup');
				...
				if (i === 1) { // Retrieve amount of cols from the first row
