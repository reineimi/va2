...
va2.mod.box = {
	...
	*/
		...
		['Source', {ph:'path or URL', tags:['img', 'audio', 'source', 'track', 'script']}],
		...
		['Arrangement (flex)', {hint:'Content arrangement method within the (flex) box.'}],
		['Width'],
	...
	loop(va2.mod.box.attrs, (i,v)=>{
		...
		if (type(va2.mod.box.classes[i][0]) !== 'array') {
			...
			if (enabled) {
				...
				if (args.hint) {
					create('p', {
						className: 'gicon btn fs1-5 px1 borderFg m mr0',
						innerHTML: 'info_i',
						title: args.hint || '',
