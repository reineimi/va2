...
va2.mod.box = {
	...
	*/
		...
		['Box type', {suf: {G:'g'}, hint:'Alternatively, "display" attribute. Different types have different content arrangement methods.'}],
		['Text font'],
		['Text size'],
...
// Element selector
mk('va2ctxtMenu', `<hr>
	<div class='va2 w tc mb1'>Object list (<c>Va2Box</c>):</div>
	<div id='va2box_ctxt' class='w hr10-m scroll center tc btn-g block-g m1-g br05-g'></div>
	<div class='va2 w tc m1 pt1 fcx fs08'>
		Note: This is a list of all the elements found under the cursor,
		sorted from closest (top left) to farthest (bottom right).
		<br>Va2-specific objects are not included in the list.
	</div>`);
ctxt(qem('main')[0] || document.body, ()=>{
	wipe('va2box_ctxt');
	loop(inspect(), (_,em)=>{
		let em_uid = em.dataset.uid;
		if (!em.dataset.uid) { em_uid = uid('obj_'); em.dataset.uid = em_uid; }
		mk('va2box_ctxt', `<p class='va2' onclick="va2.mod.box.edit('${em_uid}'); hide('va2ctxt')">${em.tagName}: <x class='fcx'>${em.id || em.className || ''}</x> <c>${em_uid}</c></p>`);
	});
});
...
va2.mod.box.edit = function(em_or_uid){
	...
	const win = windows.add(
		...
		"<p class='mb05 fcx fs08 tc'>Don't know what to do? Click on the <x class='gicon btn px05 i0 fc'>info_i</x> button in the window header!</p>",
	...
	loop(va2.mod.box.attrs, (i,v)=>{
		...
		if (enabled) { // Ain't gonna TAB this, too long
...
the <x class='btn green cwhite fs06'>Append</x> button, you create a new object within the current one.</p>
...
<hr class='mt2'><p class='fs1-6'>Buttons</p>
	...
	<p><x class='btn red cwhite fs06'>Remove</x> - Delete the object (cannot be undone).</p>
	<p><x class='btn skyblue cwhite fs06'>Change</x> - Change the type (tag) of the object. For example, use "input" tag to create text input.</p>
	<p><x class='btn yellow cwhite fs06'>Relocate</x> - Move the current object to the different (parent) container.</p>
	<p><x class='btn green cwhite fs06'>Append</x> - Append (create) a new object within the current one. A new window will open for the created object.</p>
	
	<p class='fs1-3 c'>Info, Clear:</p>
	<p>Button <x class='gicon btn p05 br05'>close</x> stands for "Clear" and removes all current classes associated with the parameter.</p>
	<p>Button <x class='gicon btn p05 br05'>info_i</x> stands for "Info" and displays information about the parameter on hover/click.</p>

	<p class='fs1-3 c'>Global, Hover:</p>
	<p>Button <x class='btn px1 py0 br05'>G</x> stands for "Global" and applies style for the children elements.</p>
	<p>Button <x class='btn px1 py0 br05'>H</x> stands for "Hover" and applies style when the cursor is hovering over the object.</p>
	<p>Button <x class='btn px1 py0 br05'>GH</x> works the same as <x class='btn px1 py0 br05'>H</x> for <x class='btn px1 py0 br05'>G</x>.</p>
	<p>Therefore, to apply styles for both the object and its children, first select the property and click <x class='btn px1 py0 br05'>G</x>, and then change the property for the container object.</p>

	<p class='fs1-3 c'>Maximum/Minimum:</p>
	<p>Button <x class='btn px1 py0 br05'>M</x> stands for "Maximum",
	<x class='btn px1 py0 br05'>Mi</x> for "Minimum". Both set the limit for the width/height, making width/height impossible to go beyond that limit.</p>
