...
const extern = {
	js: async function(data_or_path_or_url) {
	...
	css: function (data_or_path_or_url) {
		const str = data_or_path_or_url;
	...
	file: async function(path_or_url, _is_json) {
		let filedata;
		try {
			await fetch(path_or_url)
			.then((r_file)=>{
				let f;
				if (_is_json) {
					f = r_file.json();
...
// Promise simplified
// ex:  await wait(500, ()=>{}, ()=>{});
function wait(ms, _func_before, _func_after) {
...
// Copy contents of one obj/arr to another (for DOM use merge())
...
function em_err(func_name, em_id) {
	log(2, `${func_name} :: Element with id "${em_id}" does not exist`);
}
...
function emi(id_or_em) {
	if (typeof id_or_em === 'object') {
		return id_or_em;
	} else if (document.getElementById(id_or_em)) {
		return document.getElementById(id_or_em);
	} else {
		return null;
	}
}
...
function emtoggle(id_or_em, _func) {
	let em = emi(id_or_em);
...
function emroot(id_or_em, level) {
	let em = emi(id_or_em);
	if (em && em.tagName) {
		let str = '.parentNode';
		return eval('em'+str.repeat(level));
	} else {
		log(2, 'emroot :: Element not found');
	}
}
function emitem(id_or_em, number) {
	let em = emi(id_or_em);
	if (em && em.tagName) {
		return em.children[number];
	} else {
...
function emitems(id_or_em, _func) {
...
// Change root or return it back
function emchroot(id_or_em, root_id_or_em, _switch_between) {
	let em = emi(id_or_em);
	let root = emi(root_id_or_em);
	if (em) {
		...
		em_err('em_chroot', tostring(id_or_em));
	}
}

// Return calculated properties of the element
function embox(id_or_em) {
	const em = emi(id_or_em),
...
// <str/em> root - element, or ID of (0 to use document.body)
// <str/em> data - HTML string or element
function mk(root, ...data) {
	let em;

	if ((typeof root === 'string') && emi(root)) {
		em = emi(root);
	} else if (root && root.tagName) {
		em = root;
	} else {
		em = document.body;
	}
...
function create(tag, _data, _root_emi) {
	let em = document.createElement(tag);
	if (_data) {
		loop(_data, (i,v)=>{
			em[i] = v;
		});
...
function getclass(id, classname) {
	return emi(id).classList.contains(classname);
}

// Loop through a class using:
// classes(class).loop((_,em)=>{ ... });
...
function tags(id, tag) {
	if (emi(id)) {
		return [...emi(id).querySelectorAll(tag)];
	} else {
		em_err('tags',  id);
	}
}
...
function hshow(root, display, ...ids) {
	if (emi(id)) {
		emi(root).onmouseover = ()=>{
			loop(ids, (_, id)=>{ va2.show(display, id); });
		}
		emi(root).onmouseout = ()=>{
			loop(ids, (_, id)=>{ va2.hide(id); });
		}
	} else {
		em_err('hshow', id);
	}
...
function text(id, ...data) {
	if (emi(id)) {
		emi(id).textContent = '';
		loop(data, (_, txt)=>{
			emi(id).textContent = emi(id).textContent + txt + '<br>';
		});
	} else {
		em_err('text',  id);
	}
...
function links(html_str) {
	...
	loop(links, (_,v)=>{
		html_str = html_str.replace(v[1], `<a href='${v[1]}' target='_blank'>${v[1]}</a>`);
	});
	return html_str;
}

// Format hexadecimal colors
function hexcol(str) {
	const hex = str.match(/((0x|(?<!&)#)[0-9a-fA-F]{3,8})/gm);
	loop(hex, (_,v)=>{
		let cssv = v.replace('0x', '#');
		str = str.replaceAll(v, `<i style='color: ${cssv}'>${v}</i>`);
	});
	return str;
}

// Input: On Enter key press event (passes em to func)
function enter(id_or_em, func) {
	const em = emi(id_or_em);
	if (em) {
		if (!em.uid) {
			em.uid = uid('__input::');
...
function scrollto(id_or_em_or_x_coord, _y_coord) {
	const dest = id_or_em_or_x_coord;
	if (emi(dest)) {
...
function ani(id, animation, _timer_sec, _hide) {
	const em = emi(id);
	if (em) {
		em.style.animation = null;
		void emi(id).offsetWidth; // Animation reflow
...
// <p onclick="va2.slide('cat', 'n')">cats</p>
// <img id='cat1'> <img id='cat2'>
function slide(id, dir) {
	const slides = va2.temp.slides;
...
// Open URL:  href('...', true);
function href(https, openInNewTab) {
	if (openInNewTab) { window.open(https, '_blank'); }
...
function vport(func_desktop, func_mobile) {
	...
	// Height > width by >75%
	if (Math.per(h, w) > 75) { func_mobile(); }
	else { func_desktop(); }
}

// Copy text from the selected input or string
function copy(id_or_str) {
	let data;
	if (emi(id_or_str)) {
...
Object.prototype.chroot = function(root_em_or_id, _switch_between) {
	return emchroot(this, root_em_or_id, _switch_between);
}
