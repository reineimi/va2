...
"use strict";
const va2 = {
	ver: '1.85.81.208', // rebase(major) . changes(minor) . additions . fixes
	verTips: [
...
function popup(content, _title) {
	...
	if (!em) {
		...
		<div id='va2popupWrap' class='fillx f p2'>
			...
			<div class='m w1-3 vh60-m p2 fgc bfgi b1px br shadow scroll ani-popUp ani05'>
...
windows.terminal = function() {
  ...
  if (!emi('va2term')) {
	...
	create('p', {
		...
		onclick: ()=>{
			popup(
			`<div class='fontCode p2 br bfgi b1px bgc lh13'>${va2.temp.termHist.join('<br>')}</div>
	...
	create('p', {
		...
		onclick: ()=>{
			popup(`<div class='f'><textarea class='w bgc bbgi b1px vh40 mb2 p2 fontCode br1 fs09'></textarea>
		...
		<div id='va2ctxt' class='fillx hide f'>
			...
			<div id='va2ctxtMenu' class='w1-3 m p1 th_windowBg border br scroll shadow'>
				<div class='va2 w center mx05-g my1-g br05-g py1-g px2-g'>
					...
					<p class='va2 btn btn_notifs' onclick='notifs.show()' title='Notifications'><gicon class='va2'>notifications</gicon></p>
				</div>
			</div>
		</div>
	`);
