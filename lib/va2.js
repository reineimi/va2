...
// root: Element or id of (0 to use document.body)
// data: HTML string(s) or element(s)
...
function tags(em_or_id, tag) {
	if (emi(em_or_id)) {
		return [...emi(em_or_id).querySelectorAll(tag)];
	} else {
		em_err('tags',  em_or_id);
	}
}
...
function text(em_or_id, ...data) {
	if (emi(em_or_id)) {
		emi(em_or_id).textContent = '';
		loop(data, (_, txt)=>{
			emi(em_or_id).textContent =
			emi(em_or_id).textContent + txt + '<br>';
		});
	} else {
		em_err('text',  em_or_id);
	}
}
...
function ani(id_or_em, animation, _timer_sec, _hide) {
	const em = emi(id_or_em);
	if (em) {
...
function copy(id_or_em_or_str) {
	let data;
	if (emi(id_or_em_or_str)) {
		const em = emi(id_or_em_or_str);
		em.select();
		em.setSelectionRange(0, 99999);
		data = em.value;
	} else {
		data = id_or_em_or_str;
	}
