...
"use strict";
const va2 = {
	ver: '1.84.80.204', // rebase(major) . changes(minor) . additions . fixes
	verTips: [
	...
	init: function() {
		...
		`<div id='va2notifs_wrap' class='fillx f pt1 pb6 hide'>
			...
			<div class='th_windowBg border m p2 round h fh'>
				...
				${showinfo}
				<div id='va2notifs_nav' class='m f pt2 px2-g g1 bT fs1-2 w jce'>
					${notifs.nav.join('')}
					<div class='w f p0'>
	...
	add: function(text, _classes, _action) {
		...
		const _notif = create(0, {
			innerHTML: `<div class='m p2 th_windowFg border round f text ts-hover1 ani-slideFromT ts02 ${_classes}'>`+
				...
				"<p class='botR btn center fc' onclick='notifs.del(this.root(2))'>Close</p></div>",
		}, em);
		if (_action) {
			addclass(_notif, 'pt');
			_notif.onclick = ()=>{
...
va2.mod.temp.init = ()=>{
	...
	#va2notif { z-index: 88; top: var(--headerH); left: 0px; transform: translateY(-150%); }
		...
		<div id='va2ctxt' class='fillx hide f'>
			...
			<div id='va2ctxtMenu' class='w1-3 m p1 th_windowBg border br scroll shadow'>
				<div class='va2 w center mx05-g my1-g br05-g py1-g px2-g'>
					<p class='va2 btn' onclick='window.location.reload()' title='Refresh page'><gicon class='va2'>refresh</gicon></p>
				</div>
			</div>
		</div>
	`);
	...
	mk('va2ctxtMenu', "<hr><div id='va2ctxtItems' class='w tc table btn-g block-g m1-g py1-g br05-g'></div>");
