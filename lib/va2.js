...
"use strict";
const va2 = {
	ver: '1.2.35.151', // (major.minor.fixes.commits)
	verTips: [
		"Va2: There's a new major release. It might be necessary to update your current "+
		"project in order to fix compatibility issues. Please check the release here: https://github.com/reineimi/va2",
		"Va2: There's a new minor release. It's recommended to check your project for compatibility issues. "+
		"Please head over here: https://github.com/reineimi/va2",
		"Va2: There has been new fixes/improvements in the library. You can take a look at them here: https://github.com/reineimi/va2"
	],
...
function checkver() {
	...
	if (mtag) {
		console.log('Va2 local version:	'+mtag.dataset.ver);
		const ver_numbers = [];
		loop(va2.ver.matchAll(/[0-9]+/g), (i,v)=>{ ver_numbers[i] = Number(v); });
		loop(mtag.dataset.ver.matchAll(/[0-9]+/g), (i,v)=>{
			if (Number(v) < ver_numbers[i]) {
				if (va2.verTips[i]) { console.log('%c'+va2.verTips[i], va2.verTipsStyle); }
				return 1;
			}
		});
	}
...
//	fn/func: Function to execute with each file (file_src, root_em)=>{}
			...
			loop(em.files, (i,v)=>{
				...
				if (_root) {
					...
					reader.addEventListener('load', ()=>{
						...
						if (args.fn) { args.fn(reader.result, _root); }
						if (args.func) { args.func(reader.result, _root); }
					});
...
// Shows "va2notif" element with specified text
...
// Shows "va2prompt" element with specified text;
