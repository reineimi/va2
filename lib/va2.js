...
"use strict";
const va2 = {
	ver: '1.75.68.194', // rebase(major) . changes(minor) . additions . fixes
	verTips: [
...
function inspect(_x, _y) {
	...
	if (document.elementsFromPoint) {
		...
		loop(queue, (i,v)=>{
			if ((v.tagName!=='HTML') && (v.tagName!=='BODY') && (v.tagName!=='MAIN') && (!v.id || !v.id.match('va2'))) {
...
const notifs = {
	...
	nav: [
		...
		"<p class='btn' onclick='setclass(document.body, \"noani\")' title='Disable animations'><gicon>motion_mode</gicon></p>",
	...
	add: function(text, _classes, _action) {
		...
		const _notif = create(0, {
			innerHTML: `<div class='m p2 th_windowFg border round f text ts-hover1 ani-slideFromT ts02 ${_classes}'>`+
				...
				"<p class='botR btn px2 center' onclick='notifs.del(this.root(2))'>Close</p></div>",
		}, em);
		if (_action) {
			addclass(_notif, 'pt');
			_notif.onclick = ()=>{
...
const windows = {
	recent: null, all: [],
	// Create window, returns: [window, windowBody]
	add: function(_title, _content) {
		const the_uid = uid()
		const em = create(0, {
			className: 'va2window m w1-3 h50 p1',
			innerHTML: `
			<div class='int w h th_windowBg br bounded border shadow'>
				<div class='w py1 px2 f th_windowFg'>
					<div class='fs1-2 m ml0 pr3'>${_title || "Window ID: "+the_uid}</div>
					<div class='fs1-2 m mr0'>
						<div class='gicon btn' onclick='hide(emroot(this, 4))'>minimize</div>
						<div class='gicon btn' onclick='windows.close(emroot(this, 4))'>close</div>
					</div>
				</div>
	...
	terminal: ()=>{
		...
		const inp = create('input', {
			className: 'w input bn br0 p2 pt0 hr3 th_windowFg',
			placeholder: '(javascript input area)'
		}, emitem(root, 1));
		enter(inp, ()=>{ eval(inp.value); inp.value = ''; });
		rm(qem('#va2term .btn')[1]);
	},
}
...
va2.mod.temp.init = ()=>{
	...
	#va2ctxt { z-index: 91; }
	#va2ctxt > div:first-child { background: #0007; }
	...
	mk(document.body, `
		...
		<div id='va2ctxt' class='fillx hide f'>
			<div id='va2ctxtEsc' class='fill ani-fadeout' onclick='hide(this.root())'></div>
			<div id='va2ctxtMenu' class='w1-3 m p1 th_windowBg border br scroll shadow'>
				<div class='w center'>
					<p onclick='window.location.reload()' title='Reload page'><gicon>directory_sync</gicon></p>
				</div>
			</div>
		</div>
	`);
	if (emi('Main')) {
		emchroot('va2windowArea', 'Main');
	}
	windows.terminal();
