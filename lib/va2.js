...
"use strict";
const va2 = {
	ver: '1.6.46.163', // (major.minor.fixes.commits)
	verTips: [
...
// sec: Show for N seconds
// func: Function to trigger on click
function notify(text, _classes, _args) {
	const args = _args || {};
	if (!va2.opt.dnd) {
		let sec = args.sec || 2 + (text.length * 0.05);
		const id = 'va2notif';
		...
		em.innerHTML = `<div class='m shadow py2 px3 th_windowBg round fcx text f pt ${_classes}' onclick='notifs.show()'><div class='m pt1-g'>${text}</div></div>`;
	}
...
const notifs = {
	...
	init: function() {
		...
		`<div id='va2notifs_wrap' class='fillx f hide'>
			<div class='fillx' style='background:#0007' onclick='notifs.show()'></div>
			<div class='th_windowBg border m p2 round f'>
				<p class='w tc fcx pb2 bb fs1-5'>Notifications</p>
				<div id='va2notifs' class='scroll my2-g w z1'></div>
				<div id='va2notifs_nav' class='m pt2 px2-g g1 bt center fs1-2'>
					<p class='btn cred' onclick='notifs.clear()' title='Clear notifications'><gicon>delete</gicon> Clear</p>
					<p class='btn btn_dnd' onclick='va2.f.dnd()' title='Do Not Disturb'><gicon>notifications_off</gicon></p>
					${notifs.nav.join('')}
					<p class='btn' onclick='notifs.show()' title='Close notification tray'><gicon>close</gicon> Close</p>
				</div>
			</div>
		</div>`);
		notifs.clear();
	...
	add: function(text, _classes, _action) {
		...
		if (em.item(0).tagName === 'P') { rm(em.item(0)); }
		const _notif = create(0, {
			innerHTML: `<div class='m p2 th_windowFg border round fcx f text ${_classes}'>`+
				`<div class='mb1 mt1-g'>${text}</div>`+
				"<p class='botr btn px2 fcx' onclick='notifs.del(this.root(2))'>Close</p></div>",
		}, em);
		if (_action) {
			addclass(_notif, 'pt');
			_notif.onclick = ()=>{
	...
	clear: function() {
		...
		emi('va2notifs').innerHTML = '<p class="tc">The list is empty</p>';
	}
}
va2.f.dnd = function() {
	...
	if (va2.env.verbose) {
		if (va2.opt.dnd) { notify('Do Not Disturb turned ON', 'red cwhite'); }
		else { notify('Do Not Disturb turned OFF', 'blue cwhite'); }
	}
}
...
function stdin(text, _func_on_enter) {
	...
	if (em.item(0)) { em.item(0).textContent = text; }
	if (em.item(1)) {
		enter(em.item(1), ()=>{
			if (_func_on_enter) {
