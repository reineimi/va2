...
"use strict";
const va2 = {
	ver: '1.84.79.203', // rebase(major) . changes(minor) . additions . fixes
	verTips: [
...
function popup(content, _title) {
	...
	if (!em) {
		...
		<div id='va2popupWrap' class='fillx f p2'>
			...
			<div class='m w1-3 vh60-m p2 th_windowBg borderFg br shadow scroll ani-popUp ani05'>
				<div class='btn gicon fs1-2 px1 abs topR m2 z2' onclick='hide(emroot(this, 2))'>close</div>
				<div id='va2popupTitle' class='tc pb2 pt05 px5 mb05 lh12'></div>
				<div id='va2popup'></div>
			</div>
		</div>`);
		em = emi('va2popup');
...
windows.terminal = function() {
  ...
  if (!emi('va2term')) {
	...
	const inp = create('input', {
		className: 'input br0 py0 hr3 th_windowBg shadowS f1',
	...
	create('p', {
		...
		onclick: ()=>{
			popup(
			`<div class='fontCode p2 br bbgi lh13'>${va2.temp.termHist.join('<br>')}</div>
	...
	create('p', {
		...
		onclick: function(){
			setclass(this, 'blue', 'blue-hov', 'cwhite');
	...
	create('p', {
		...
		onclick: ()=>{
			popup(`<div class='f'><textarea class='w bbgi b1px vh40 mb2 p2 fontCode br1 fs09'></textarea>
