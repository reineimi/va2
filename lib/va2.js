...
"use strict";
const va2 = {
	ver: '1.76.68.197', // rebase(major) . changes(minor) . additions . fixes
	verTips: [
	...
	env: { // "Global environment"
		init: [],
		v: {}, // Storage for vars
		verbose: 1, // Prints and on-screen notifications for a lot of things
		info: 1, // Shows various information in various places
		proto: 0, // Enable Prototypes
		mods: 1 // Enable Modules
	},
	opt: { // Frontend options (settings)
		dnd: false, // Notifications: Do Not Disturb
		lang: 'EN', // Current content language
		langs: { // Available languages
			EN:1, ES:1, FR:1, RU:1, CN:1, KR:1, JP:1,
		}
	},
	mod: {}, // Modules
	e: {}, // Storage for enums
		...
		loop(data.ver.matchAll(/[0-9]+/g), (i,v)=>{
			if (Number(v) < ver_numbers[i]) {
				if (va2.verTips[i]) {
					...
					if (va2.env.verbose) {
						notify(va2.verTips[i], 'cwhite yellow a-ul');
					}
				}
				return 1;
			}
...
//	(bool) mu/mult: Select multiple files
//	(em_i) em/root: Element to insert files to
//	(func) fn/func: Function to execute with each file (args{em, root, src})=>{}
//	(str)  cl/css:  CSS classes added to files  */
...
function themeset(_theme_name) {
	...
	if (_theme_name) {
		...
		if (themes[_theme_name]) { setclass(root, id); }
		themes.loop((i,v)=>{
			if ('theme-'+i !== id) { rmclass(root, 'theme-'+i); }
		});
	} else {
...
function va2lang(lang_short) {
	lang_short = lang_short.toUpperCase();
	if (va2.opt.langs[lang_short]) {
		va2.opt.lang = lang_short;
		document.documentElement.lang = lang_short.toLowerCase();
	}
...
function popup(content, _title) {
	if (content) {
		const wrap = create(0, {
			className: 'fillx va2popup f p2',
			innerHTML: "<div class='fill transparent' onclick='rm(emroot(this))'></div>"
		});
		const em = create(0, {
			className: 'm w1-3 vh60 p2 th_windowBg borderFg br shadow scroll',
			innerHTML: "<div class='btn gicon fs1-2 px1 abs topR m2 z2' onclick='rm(emroot(this, 2))'>close</div>"
		}, wrap);
		if (_title) {
			mk(em, `<div class='w tc pb2 lh12'>${_title}</div>`);
		}
		mk(em, content);
	}
}
...
function stdin(text, _func_on_enter) {
	const _wrap = emi('va2prompt_wrap') || create(0, {
		id: 'va2prompt_wrap',
		className: 'fillx f z9 p3',
		innerHTML: "<div class='f fill' style='background:#0007' onclick='hide(this.root())'></div>"+
			"<div id='va2prompt' class='m p2 th_windowFg round w1-3'>"+
			"<p class='tc pb2'></p><input class='input i w tc' type='text' placeholder='(input area)'></div>"
	}, document.body);

	const em = emitem(_wrap, 2);
	if (emitem(em, 1)) { emitem(em, 1).innerHTML = text; }
	if (emitem(em, 2)) {
		enter(emitem(em, 2), ()=>{
			if (_func_on_enter) {
				_func_on_enter(emitem(em, 2).value);
			}
			emitem(em, 2).value = '';
			hide(_wrap);
		}, true);
...
function notify(text, _classes, _args) {
	...
	if (!va2.opt.dnd) {
		...
		em.innerHTML = `<div class='m shadow py2 px3 th_windowBg round text f pt border ${_classes}' onclick='notifs.show()'><div class='m pt1-g'>${text}</div></div>`;
	}
...
const notifs = {
	list: {},
	nav: [
		"<p class='btn' onclick='show(\"va2term\"); hide(\"va2ctxt\"); notifs.hide()' title='Open terminal'><gicon>terminal</gicon></p>",
		"<p class='btn btn_dnd' onclick='va2.f.dnd()' title='Do Not Disturb'><gicon>notifications_off</gicon></p>",
		"<p class='btn' onclick='fullscreen()' title='Fullscreen mode'><gicon>open_in_full</gicon></p>",
		"<p class='btn btn_noani' onclick='va2.f.noani()' title='Disable animations'><gicon>motion_mode</gicon></p>",
		"<p class='btn' onclick='themeset()' title='Switch theme'><gicon>contrast</gicon></p>",
		"<p class='btn' onclick='notify(\"Settings are not implemented yet.\", \"red cwhite\"); hide(\"va2ctxt\")' title='Settings'><gicon>settings</gicon></p>",
	],
	clear_texts: [
		'The list is empty.',
		'Wow! Such emptiness...',
	...
	init: function() {
		const _meta = qem('meta[name="va2meta"]')[0];
		let mdata = {};
		if (_meta) { mdata = JSON.parse(_meta.content); }
		let showinfo = '';
		if (va2.env.info) {
			showinfo =
		...
		`<div id='va2notifs_wrap' class='fillx f pt1 pb6 hide'>
			...
			<div class='th_windowBg border m p2 round h fh'>
				...
				<div id='va2notifs' class='scroll my2-g w z1 f1'></div>
				${showinfo}
	...
	add: function(text, _classes, _action) {
		...
		const _notif = create(0, {
			innerHTML: `<div class='m p2 th_windowFg border round f text ts-hover1 ani-slideFromT ts02 ${_classes}'>`+
				...
				"<p class='botR btn px2 center fc' onclick='notifs.del(this.root(2))'>Close</p></div>",
		}, em);
		if (_action) {
			addclass(_notif, 'pt');
			_notif.onclick = ()=>{
...
const windows = {
	...
	/* Create a window, returns: [window, windowBody]; args:
		(str_html) info: display information button with a popup
	...
	*/
	add: function(_title, _content, _args) {
		...
		const the_uid = uid();
		const em = create(0, {
			className: 'va2window m mx0 w1-3 h50 p1',
			innerHTML: `
			<div class='int w h th_windowFg br bounded borderFg shadowX'>
				<div class='w py1 px2 f th_windowFg bB shadowS'>
					<div class='fs1-2 m ml0 pr3'>${_title || "Window ID: "+the_uid}</div>
					<div class='fs1-2 f m mr0 g05'>
				...
				<div class='va2windowBody w p2 scroll'>${_content || ''}</div>
			</div>`,
		}, 'va2windowArea');
		em.dataset.windowId = the_uid;
		windows.recent = em;
		windows.all.push(em);
	...
	terminal: ()=>{
		...
		root.id = 'va2term';
		addclass(root, 'hide');
		addclass(em, 'va2term', 'text', 'fontCode', 'tl', 'lh12');
		em.innerHTML = va2.temp.term.join();
		const inp = create('input', {
			className: 'w input br0 p2 pt0 hr3 th_windowFg shadowS',
			placeholder: '(javascript input area)'
		}, emitem(root, 1));
		enter(inp, ()=>{ eval(inp.value); inp.value = ''; });
		//rm(qem('#va2term .btn')[2]);
	},
}
...
va2.mod.temp.init = ()=>{
	...
	mk(document.head, `
	<style>
	#va2popup { z-index: 87; animation: popUp 0.5s forwards; }
	#va2prompt { z-index: 89; animation: slideFromT 0.5s forwards; }
	#va2notif { z-index: 88; top: var(--headerH); left: 0px; }
	#va2notif { transform: translateY(-150%); }
	#va2notif > div, #va2prompt > * { max-width: 90vmin; }
	#va2notif img { display: block; width: 10rem; }
	#va2notifs_wrap { z-index: 89; }
	...
	#va2notifs_nav { justify-content: end; }
	div:has(> #va2notifs) { animation: appearFromT 0.25s; }
	#va2ctxt { z-index: 91; }
	#va2ctxtMenu { max-width: 80vmin; max-height: 40rem; animation: slideFromB 0.2s; }
	#va2ctxtMenu p, #va2ctxtMenu a { display: block; border-radius: 0.5rem; background: var(--itemBg); margin: 0.5rem; padding: 0.5rem 1rem; cursor: pointer; }
	#va2ctxtMenu p:hover, #va2ctxtMenu a:hover { background: var(--itemFg); }
	#va2ctxtMenu hr { margin: 0.5rem 0px; border-color: var(--itemBg); }
	.va2windowBody { height: calc(100% - 3rem); }
	.va2term { height: calc(100% - 5rem); }
	</style>`);

	// Create core elements
	mk(document.body, `
		<div id='va2windowArea' class='fillx center z1 scroll nil'></div>
		<div id='va2ctxt' class='fillx hide f'>
			<div id='va2ctxtEsc' class='fill ani-fadeout transparent' onclick='hide(this.root())'></div>
			<div id='va2ctxtMenu' class='w1-3 m p1 th_windowBg border br scroll shadow'>
				<div class='w center'>
					<p onclick='window.location.reload()' title='Refresh page'><gicon>refresh</gicon></p>
				</div>
			</div>
		</div>
	`);
	if (emi('Main')) { emchroot('va2windowArea', 'Main'); }
	windows.terminal();

	// Context menu: Add general buttons
	loop(notifs.nav, (_,em)=>{ mk(emitem('va2ctxtMenu', 1), em); });
	mk('va2ctxtMenu', "<hr><div id='va2ctxtItems' class='w tc table'></div>");
...
va2.env.init[0] = ()=>{
	...
	loop(qem('.time'), (_,em)=>{
		const U = uid('time-');
		em.dataset.uid = U;
		interval.set(U, ()=>{ em.innerHTML = time(); }, 1);
	});
	loop(qem('.date'), (_,em)=>{
		const U = uid('date-');
		em.dataset.uid = U;
		interval.set(U, ()=>{ em.innerHTML = date(); }, 1);
	});
	loop(qem('.timedate'), (_,em)=>{
		const U = uid('tdate-');
		em.dataset.uid = U;
		interval.set(U, ()=>{ em.innerHTML = timedate(); }, 1);
	});
