...
"use strict";
const va2 = {
	data: {}, // Storage for accessible userdata. PLEASE STORE YOUR MOD DATA HERE
	ver: '1.87.89.209', // rebase(major) . changes(minor) . additions . fixes
	verTips: [
...
va2.f.saveData = ()=>{ // Save accessible data
	const data = {}
...
va2.f.loadData = ()=>{ // Load accessible data
...
function stdin(func, _text) {
	...
	if (!em) {
		...
		<div id='va2promptWrap' class='fillx f p3'>
			<div class='f fill' style='background:#0007' onclick='hide(emroot(this))'></div>
...
windows.settings = function(_custom_elements) {
  ...
  if (!emi('va2settings')) {
	...
	mk(btnList, "<p class='green cwhite' onclick='va2.f.saveData(); notify(\"Data saved!\", \"green cwhite\")'>Save data</p>");
	create('P', {
		className: 'red cwhite',
		innerHTML: 'Erase data',
		onclick: ()=>{
			stdin((v)=>{ if (v==='yes'){ storage.wipe(); location.reload(); } },
			'To erase all data, write "<x class="cred">yes</x>" in the field below.')
		}
	}, btnList);
...
va2.mod.temp.init = ()=>{
		...
		<div id='va2ctxt' class='fillx hide f'>
			...
			<div id='va2ctxtMenu' class='w1-3 m p1 th_windowBg border br scroll shadow'>
				<div class='va2 w center mx05-g my1-g br05-g py1-g px2-g'>
					...
					<p class='va2 btn btn_notifs' onclick='notifs.show(); hide("va2ctxt")' title='Notifications'><gicon class='va2'>notifications</gicon></p>
				</div>
			</div>
		</div>
	`);
	...
	mk('va2ctxtMenu', "<hr><div id='va2ctxtItems'"+
	" class='w tc table btn-g block-g m1-g py1-g br05-g'></div>");
