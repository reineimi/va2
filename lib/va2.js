...
"use strict";
const va2 = {
	ver: '1.74.66.192', // rebase(major) . changes(minor) . additions . fixes
	verTips: [
...
function stdin(text, _func_on_enter) {
	...
	if (emitem(em, 1)) { emitem(em, 1).textContent = text; }
	if (emitem(em, 2)) {
		enter(emitem(em, 2), ()=>{
			if (_func_on_enter) {
...
const windows = {
	...
	add: function(_title, _content) {
		const the_uid = uid()
		const em = create(0, {
			className: 'va2window m w1-6-3 h50 p1',
			innerHTML: `
			<div class='int w h th_windowBg br bounded border shadow'>
				<div class='w py1 px2 f th_windowFg nosel'>
	...
	terminal: ()=>{
		...
		windows.all.pop();
		root.id = 'va2term';
		addclass(root, 'va2term', 'hide');
		addclass(em, 'text', 'fontCode', 'tl');
...
va2.mod.temp.init = ()=>{
	...
	.va2term .va2windowBody { height: calc(100% - 5rem); }
	</style>`);

	// Create core elements
	mk(document.body, `
		<div id='va2windowArea' class='fillx center nil'></div>
