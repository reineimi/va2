...
"use strict";
const va2 = {
	ver: '1.82.74.202', // rebase(major) . changes(minor) . additions . fixes
	verTips: [
...
// Initial variables
va2.sess.id = uid(date('')+'_');
va2.sess.date = timedate();
		...
		loop(ver.matchAll(/[0-9]+/g), (i,v)=>{
			if (Number(v) < ver_numbers[i]) {
				if (va2.verTips[i]) {
					...
					if (va2.opt.verbose) {
						notify(va2.verTips[i], 'cwhite brown a-ul');
					}
				}
				return 1;
			}
...
function popup(content, _title) {
	...
	if (!em) {
		...
		<div id='va2popupWrap' class='fillx f p2'>
			<div class='fill transparent' onclick='rm(emroot(this))'></div>
			<div class='f m w1-3 vh60-m p2 th_windowBg borderFg br shadow scroll ani-popUp ani05'>
				<div class='btn gicon fs1-2 px1 abs topR m2 z2' onclick='rm(emroot(this, 2))'>close</div>
				<div id='va2popupTitle' class='w tc pb2 pt05 lh12'></div>
				<div id='va2popup' class='m'></div>
			</div>
		</div>`);
		em = emi('va2popup');
	}
...
function stdin(func, _text) {
	if (!func) { return false; }
	let em = emi('va2prompt');
	if (!em) {
		mk(0, `
		<div id='va2promptWrap' class='fillx f z9 p3'>
			<div class='f fill' style='background:#0007' onclick='hide(this.root())'></div>
			<div id='va2prompt' class='m p2 th_windowFg round w1-3 ani-popUp ani05'>
				<p class='tc pb2'></p>
				<input class='input i w tc' type='text' placeholder='(input area)'>
			</div>
		</div>`);
		em = emi('va2prompt');
	}
	const text = emitem(em, 1);
	const inp = emitem(em, 2);
	text.innerHTML = _text ||
	"User input prompt<br><x class='fs08 fcx'>(This data will be processed by the associated function)";
	enter(inp, ()=>{
		func(inp.value);
		inp.value = '';
		hide('va2promptWrap');
...
const notifs = {
	list: {},
	nav: [
		"<p class='va2 btn' onclick='windows.terminal()' title='Open terminal'><gicon class='va2'>terminal</gicon></p>",
		"<p class='va2 btn btn_dnd' onclick='va2.f.dnd()' title='Do Not Disturb'><gicon class='va2'>notifications_off</gicon></p>",
		"<p class='va2 btn' onclick='fullscreen()' title='Fullscreen mode'><gicon class='va2'>open_in_full</gicon></p>",
		"<p class='va2 btn btn_noani' onclick='va2.f.noani()' title='Disable animations'><gicon class='va2'>motion_mode</gicon></p>",
		"<p class='va2 btn' onclick='themeset()' title='Switch theme'><gicon class='va2'>contrast</gicon></p>",
		"<p class='va2 btn' onclick='windows.settings()' title='Settings'><gicon class='va2'>settings</gicon></p>",
	],
	clear_texts: [
		'The list is empty.',
		'Wow! Such emptiness...',
	...
	loop(va2.opt, (i,v)=>{
		...
		if (va2.hints.opt[i]) {
			create('p', {
				...
				onclick: ()=>{ notify(va2.hints.opt[i], 0, {silent:1}); }
			}, title);
		}
	...
	mk(document.body, `
		...
		<div id='va2ctxt' class='fillx hide f'>
			...
			<div id='va2ctxtMenu' class='w1-3 m p1 th_windowBg border br scroll shadow'>
				<div class='va2 w center'>
					<p class='va2' onclick='window.location.reload()' title='Refresh page'><gicon class='va2'>refresh</gicon></p>
				</div>
			</div>
		</div>
	`);
