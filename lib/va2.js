...
const va2 = {
	...
	ver: '1.89.97.211', // rebase(major) . changes(minor) . additions . fixes
	verTips: [
	...
	f: {}, // Storage for embeddable functions
	e: {}, // Storage for enums
...
va2.f.exportData = ()=>{
	const data = storage.loc.get('va2data');
	mkfile(va2.sess.id+'.va2.data.json', JSON.stringify(data));
}
va2.f.importData = ()=>{
	getfile((_,data)=>{
		loop(data, (i,v)=>{
...
va2.env.init[0] = ()=>{
	...
	// Color init
	if (va2.opt.accentColor) {
		cssvar('accent', `var(--${va2.opt.accentColor})`);
		cssvar('accentBg', `var(--dark${va2.opt.accentColor})`);
	}
