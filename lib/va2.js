...
const extern = {
	...
	file: async function(path_or_url_or_emiToPasteFileIn, _is_json) {
		let targ = path_or_url_or_emiToPasteFileIn;
		let filedata;
		...
		if (emi(targ)) { // Upload to element
			...
			inp.onchange = async ()=>{
				let fdata = fr.result;
				fr.onload = ()=>{
					emi(targ).innerHTML = fr.result;
					va2.temp.extern.file = fr.result;
				}
...
function emroot(id_or_em, _level) {
	let em = emi(id_or_em);
	let lvl = _level || 1;
	if (em && em.tagName) {
		let str = '.parentNode';
...
function emitem(id_or_em, _number) {
	let em = emi(id_or_em);
	let num = _number || 0;
	if (em && em.tagName) {
...
function create(tag, _data, _root_emi) {
	...
	let root = _root_emi || document.body;
	if (_data) {
		loop(_data, (i,v)=>{
			em[i] = v;
		});
