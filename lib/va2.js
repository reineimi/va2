...
"use strict";
const va2 = {
	ver: '1.80.71.201', // rebase(major) . changes(minor) . additions . fixes
	verTips: [
...
const notifs = {
	list: {},
	nav: [
		"<p class='btn' onclick='windows.terminal()' title='Open terminal'><gicon>terminal</gicon></p>",
		...
		"<p class='btn' onclick='windows.settings()' title='Settings'><gicon>settings</gicon></p>",
	],
	clear_texts: [
		'The list is empty.',
		'Wow! Such emptiness...',
		...
		const em = create(0, {
			...
			<div class='int w h th_windowFg br bounded borderFg shadowX'>
				<div class='w f py1 px2 th_windowFg bB shadowS'>
					<div class='w1-2 fs1-2 m ml0 tl pr3'>${_title || "Window ID: "+the_uid}</div>
...
windows.terminal = function() {
  ...
  if (!emi('va2term')) {
	...
	const wrap = create(0, {className: 'w f'}, emitem(root, 1));
	const inp = create('input', {
		className: 'input br0 p2 py0 hr2 th_windowFg shadowS f1',
	...
	const btnUp = create('p', {
		className: 'gicon btn py1 br0 b1px bbgi fgc bgic-hov',
...
windows.settings = function() {
  ...
  if (!emi('va2settings')) {
	...
	addclass(win, 'mb1-g');
	// Top texts
	mk(win, `<p class='abs topR py1 px2 fcx fs08'>ver: ${va2.ver}</p>`);
	mk(win, "<p class='fs2-5 py1 fontFancy'>General</p>");
	...
	const colors = create(0, {
		className:'w1-5-4 m center border b02 bc br pb1 bga g05',
	...
	mk(win, "<p class='fs2-5 pt3 pb1 lh09 fontFancy'>Configuration<br><x class='fs08 fcx font'>(va2.opt)</x></p>");
	loop(va2.opt, (i,v)=>{
		...
		if (va2.hints.opt[i]) {
			create('p', {
				...
				onclick: ()=>{ notify(va2.hints.opt[i]); }
			}, title);
		}
		...
		if (type(v)==='string' || type(v)==='number') {
			...
			// Button
			create('p', {
				className: 'i1 btn borderFg m ml0',
				innerHTML: v,
				onclick: function (){
					va2.opt[i] = va2.opt[i] ? false : true;
					this.innerHTML = va2.opt[i];
				}
			}, wrap);
